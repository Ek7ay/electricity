<template>
  <div class="FlashFood wrapper" ref="wrapper">
    <ul class="item content" ref="ulWith">
      <li v-for="(item, index) in info" :key="index">
        <img v-lazy="item.small_image"/>
        <p class="title">{{item.name}}</p>
        <div class="price-box">
          <p class="xPrice">{{item.price}}</p>
          <p class="yPrice">{{item.origin_price}}</p>
          <div class="buyCar">
...
<script>
    ...
    export default {
        ...
        computed: {
          ulWith: function () {
            return this.info.length * 2.4
          }
        },
        watch: {
          info : function (val) {
            this.$refs.ulWith.style.width = this.ulWith + "rem";
          }
        },
    }
</script>
